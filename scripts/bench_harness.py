#!/usr/bin/env python3
"""Canonical 4-card benchmark harness for DeepSeek-V4-Flash-Vision-Exp on the
SM80 vLLM fork (text path or vision path — both serve the same API shape).
Uses only the OpenAI-compatible HTTP API. Point DSV4_URL at either the
text-path or the vision-path server started by scripts/launch-*.sh.
